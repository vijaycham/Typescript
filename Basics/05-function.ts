...
performJob(logMsg);
// if we try to pass function with different type it will show error
// performJob( (num: number) => { console.log(num) } ) // error
// performJob( (msg: string) => { return msg } ) // error because it is returning something but the function type is void

type User = {
  name: string;
  age: number;
  greet: (msg: string) => void; // function type in object
  // if we want to return something from greet function we can do like below
  // greet: (msg: string) => string; // it will return string if we define like this
  }
...
let user1: User = {
  ...
  greet() {
    console.log("hello");
    return this.name; // it will show error because it is void type
  },
};
console.log(user1);
user1.greet("hi");
